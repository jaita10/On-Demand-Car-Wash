...
import { setWashPack } from "../../Actions/WashPackAction";
import { Link, useNavigate } from "react-router-dom";
import WashPackService from "../../Service/WashPackService";
import { addToTotal, setWashPackPrice } from "../../Actions/TotalAction";
...
const WashPacks = (props) => {
  ...
  const handleDelete = async (washpackId) => {
    console.log("deleting");
    console.log("deletingg......");
    const data = await WashPackService.deleteWashPack(washpackId);
    if (data === "WashPack deleted successfully") {
  ...
  const handleUpdate = (washpack) => {
    dispatch(setWashPack(washpack));
    navigate("/user/updatewashpack");
  };

  const handleSelect = (washpack) => {
    dispatch(setWashPack(washpack));
    dispatch(setWashPackPrice(washpack.washpackPrice));
    navigate("/user/book");
  };
              ...
              <p className="d-flex justify-content-between">
                {props.select ? (
                  <>
                    ...
                    <button
                      onClick={() => {
                        handleDelete(element.washpackId);
                      }}
                      type="button"
